...
public class MetricsWorker : BackgroundService
{
    ...
    private async Task BulkFlushMetricsAsync(Guid[] envIds, string[] keys, bool[] isClientFlags, long[] trueCounts, long[] falseCounts, CancellationToken ct)
    {
        ...
        await db.Database.ExecuteSqlAsync($@"
                UPDATE ""FlagEnvironmentStates"" AS fes
                SET ""TrueCount"" = fes.""TrueCount"" + upd.true_add,
                    ""FalseCount"" = fes.""FalseCount"" + upd.false_add
                FROM (
                    SELECT 
                ...
                JOIN ""ProjectFeatureFlags"" ff ON ff.""Key"" = upd.flag_key
                WHERE fes.""EnvironmentId"" = upd.env_id 
                  AND fes.""FeatureFlagId"" = ff.""Id""
                  AND (upd.is_client = FALSE OR ff.""IsClientSideExposed"" = TRUE);", ct);
    }
}