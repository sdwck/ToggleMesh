...
public class GetProfileEndpoint : ToggleEndpointWithoutRequest<UserProfileDto>
{
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        await Send.OkAsync(new UserProfileDto(user.Id, user.Email ?? "", user.UserName ?? "", user.TwoFactorEnabled, recoveryCodesLeft, user.EmailVerificationMethod, user.SkipLandingPage), ct);
    }
}
