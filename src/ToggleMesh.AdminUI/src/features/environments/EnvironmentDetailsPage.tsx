...
import { useProjectDetails, useEnvironmentKeys, useCreateEnvironmentKey, useRevokeEnvironmentKey, useUpdateEnvironment, useDeleteEnvironment } from '@/api/queries';
...
export function EnvironmentDetailsPage() {
    ...
    const csharpCode = `builder.Services.AddToggleMeshClient(options => {\n    options.BaseUrl = "${window.location.origin}";\n    options.ApiKey = "${serverKey}";\n});`;
    const typescriptCode = `import { ToggleMeshClient } from '@togglemesh/sdk';\n\nconst client = new ToggleMeshClient({\n    baseUrl: "${window.location.origin}",\n    apiKey: "${clientKey}"\n});`;

    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" onClick={() => navigate(`/projects/${projectId}/environments`)} className="cursor-pointer">
                                ...
                                <>
                                    ...
                                    {canManageKeys && (
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="start" className="border-border/40 bg-zinc-950 w-44">
                                                <DropdownMenuItem onClick={() => { setEnvNameInput(environment?.name || ''); setIsRenameOpen(true); }} className="cursor-pointer">
                                                    <Edit2 className="mr-2 h-4 w-4" /> Rename
                                                </DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => { setEnvDeleteInput(''); setIsDeleteOpen(true); }} className="text-destructive focus:text-destructive cursor-pointer">
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {isKeysLoading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={canManageKeys ? 6 : 5} className="h-24 text-center text-muted-foreground">
            ...
            <Sheet open={isSdkSetupOpen} onOpenChange={setIsSdkSetupOpen}>
                <SheetContent className="w-full sm:max-w-md overflow-y-auto bg-zinc-950 border-border/40">
                    <SheetHeader>
                    ...
                    <Tabs defaultValue="csharp" className="mt-6 space-y-4">
                        <TabsList className="bg-zinc-900 border border-border/10 h-10">
                            ...
                            <TabsTrigger value="cli" className="text-xs">CLI Tool</TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="csharp" className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <Button variant="ghost" size="icon" onClick={() => copyText(csharpCode)} className="absolute top-2 right-2 h-7 w-7 text-muted-foreground hover:text-foreground">
                        ...
                        <TabsContent value="typescript" className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <Button variant="ghost" size="icon" onClick={() => copyText(typescriptCode)} className="absolute top-2 right-2 h-7 w-7 text-muted-foreground hover:text-foreground">
                                        <Copy className="h-3.5 w-3.5" />
                                    </Button>
                                </div>
                            </div>
            ...
            <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                <DialogContent className="border-border/40 bg-zinc-950">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            This will permanently delete the environment, all of its flag configurations, and active API keys.
                            This action cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <p className="text-sm text-muted-foreground">
                            To confirm deletion, type <strong className="text-foreground font-mono">{environment?.name}</strong> below:
                        </p>
                        <Input
