...
import { useProjectDetails, useSystemConfig } from '@/api/queries';
...
export function TerminalPage() {
    ...
    return (
        <div className="space-y-4 animate-in fade-in-50 duration-500 h-full flex flex-col">
            <div className="flex flex-col flex-1 bg-zinc-950 border border-border/40 rounded-xl overflow-hidden shadow-2xl">
                ...
                <div className="flex-1 overflow-y-auto bg-[#0c0c0e] font-mono text-[13px] leading-relaxed p-4">
                    {filteredLogs.length === 0 ? (
                        sysConfig?.analyticsEnabled === false ? (
                            <div className="flex flex-col items-center justify-center h-full text-amber-500/80 space-y-2 text-center p-6">
                                <Terminal className="w-8 h-8 opacity-40 text-amber-500" />
                                <p className="text-sm font-medium text-amber-400">Live Tail Inactive</p>
                                <p className="text-xs text-zinc-500 max-w-sm">
                                    Telemetry ingestion is turned off on this server instance. No live events will be captured.
                                </p>
                            </div>
                        ) : (
