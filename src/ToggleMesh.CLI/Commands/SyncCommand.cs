...
public class SyncCommand : AsyncCommand<SyncSettings>
{
    protected override async Task<int> ExecuteAsync(CommandContext context, SyncSettings settings, CancellationToken cancellationToken)
    {
        ...
        var (configApiKey, configAddress) = ConfigResolver.ResolveFallbackConfig();
        var apiKey = settings.ApiKey ?? configApiKey;
        var address = settings.Address ?? configAddress;
        var lang = settings.Lang ?? LangDetector.DiscoverLanguage();

        if (string.IsNullOrWhiteSpace(apiKey))
        {
            AnsiConsole.MarkupLine("[red]Error:[/] API Key is missing. Provide it via --api-key or in appsettings.json.");
            return -1;
        }
        
        if (string.IsNullOrWhiteSpace(address))
        {
            AnsiConsole.MarkupLine("[red]Error:[/] API Address is missing. Provide it via --address or in appsettings.json.");
            return -1;
        }
        ...
        await AnsiConsole.Status()
            ...
            .StartAsync("Fetching flags from Control Plane...", async _ =>
            {
                flags = await apiClient.GetAllFlagsAsync(cancellationToken);
            });
        ...
        try
        {
            ...
            ICodeGenerator generator = lang.ToLower() switch
            {
                ...
                _ => throw new NotSupportedException($"Language '{lang}' is not supported yet. Select from ['typescript', 'csharp'].")
            };
            ...
            var path = settings.OutPath ?? Path.Combine(Directory.GetCurrentDirectory(), generator.DefaultFileName);
            ...
            if (removed.Count > 0)
            {
                AnsiConsole.MarkupLine("[bold red]WARNING:[/] This sync will remove some flags! Your project might not compile if they are currently in use.");

                if (!settings.AutoApply && !await AnsiConsole.ConfirmAsync("Do you want to apply these changes?", defaultValue: true, cancellationToken: cancellationToken))
                {
                    AnsiConsole.MarkupLine("[grey]Operation aborted by user.[/]");
                    return 0;
                }
            ...
            AnsiConsole.MarkupLine($"[green]✔  Success![/] Generated {mappedKeys.Count} flags to [bold]{Path.GetFileName(path)}[/]");
            return 0;
        }
        catch (Exception ex)
        {
